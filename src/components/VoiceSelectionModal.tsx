...
import { VoiceData } from '@/services/voiceService';
...
const VoiceSelectionModal: React.FC<VoiceSelectionModalProps> = ({
  ...
  const getCurrentVoiceDetails = () => {
    // Search in Eleven Labs voices
    if (allElevenLabsVoices) {
      const voice = allElevenLabsVoices.find(v => v.id === selectedVoice);
      if (voice) return transformVoiceToDisplay(voice);
    }

    // Search in legacy providers
    for (const provider in LEGACY_VOICE_PROVIDERS) {
      for (const voiceName in LEGACY_VOICE_PROVIDERS[provider as keyof typeof LEGACY_VOICE_PROVIDERS]) {
      ...
      >
        ...
        <div className="flex-1">
          ...
          {voiceData.description && (
            <p className="text-xs text-muted-foreground mt-1">{voiceData.description}</p>
          )}
              ...
              <div className="p-4 space-y-4">
                {isLoadingAll ? (
                  ...
                  <div className="text-center text-muted-foreground py-8">
                    No voices available. Please configure your Eleven Labs API key.
                  </div>
                )}
              </div>
            </ScrollArea>
            ...
            <ScrollArea className="h-[350px] w-full rounded-md border">
              <div className="p-4 space-y-4">
                {isLoadingAfrican ? (
                  ...
                  <div className="text-center text-muted-foreground py-8">
                    No African voices found. Please check your Eleven Labs API key or filters.
                  </div>
                )}
              </div>
            </ScrollArea>
